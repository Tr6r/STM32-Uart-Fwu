...
void uart_boot_send_ack()
{
	uint8_t ack = UART_BOOT_FWU_ACK;
	xprintf("%c", ack);
}

void uart_boot_send_nack()
{
	uint8_t nack = UART_BOOT_FWU_NACK;
	xprintf("%c", nack);
}
void uart_boot_rx_frame_parser(uint8_t ch)
{
	...
	case FCS_STATE:
	{
		...
		uint8_t calc = uart_boot_calcfcs(uart_boot_frame.len, uart_boot_frame.data);
		if (uart_boot_frame.fcs == calc)
		{
			if (uart_boot_frame.uart_boot_cmd_handler)
			{
				uart_boot_frame.uart_boot_cmd_handler(&uart_boot_frame);
				uart_boot_send_ack();
			}
		}
		else
		...
		uart_boot_frame.state = SOP_STATE; // reset state
	}
	break;
